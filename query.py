...
insertQuoteDetails = "INSERT INTO `quoted_items` (`quotation_no`, `item`, `item_quantity`, `quoted_price`, `quoted_items_total`) VALUES (%s, %s, %s, %s, %s);"
...
delete_qEntry = "DELETE FROM `quoted_items` WHERE `id_qitems` = %s;"
updateComment = "UPDATE `it_stock_db`.`quotations` SET `quotation_comment` = %s WHERE (`quotation_no` = %s);"
totalCostQuotedItems="UPDATE `quotations` SET `quotation_total` = (SELECT SUM(`quoted_items_total`) AS total_sum FROM `quoted_items` WHERE `quotation_no` = %s) WHERE `quotation_no`=%s;"
getQNumber="SELECT `quotation_no` FROM `quoted_items` WHERE `id_qitems`=%s;"
